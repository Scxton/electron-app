...
// Add current user ID (you'll need to get this from your auth system)
const currentUserId = ref(localStorage.getItem('userId')) // Assuming userId is stored in localStorage
...
const fetchData = async () => {
    try {
        ...
        if (testType.value === "全部") {
            results = await queryAllWithPagination(currentPage.value, pageSize.value)
            console.log("results = ", results)
        } else if (testType.value === "项目专利") {
            results = await queryAllWithPatent(currentPage.value, pageSize.value)
        } else if (testType.value === "项目论文") {
            results = await queryAllWithPaper(currentPage.value, pageSize.value)
        } else if (testType.value === "其他成果") {
            results = await queryAllWithOthers(currentPage.value, pageSize.value)
        }
        // Filter results by current user ID
        achievementData.value = results.filter(item => item.userId === currentUserId.value)
        total.value = achievementData.value.length // Update total count to reflect filtered results
    } catch (error) {
