  ...
  methods: {
    ...
    async startUpload(index) {
      ...
      try {
        ...
        const achievementName = `upload_${timestamp}`

        // 将单个文件包装成数组传递给 addFile
        await addFile([file], achievementName)
    ...
    async uploadAllFiles() {
      ...
      try {
        ...
        // 先上传所有文件
        await addFile(filesToUpload, achievementName)
        ...
        // 文件上传成功后，提交所有内容
        await this.submitAllContent()
        
        // 清理已成功上传的文件
        this.selectedFiles = this.selectedFiles.filter(
          file => file.uploadStatus !== '上传成功'
        )
      } catch (error) {
        console.error('Upload failed:', error)
        // 更新所有文件状态为失败
    ...
    async submitAllContent() {
      try {
        ...
        const achievementData = {
          ...
          intellectualPropertyId: savedFormData.intellectualPropertyId || 0,
          achievementBelongingOrganization: savedFormData.achievementBelongingOrganization || 0,
