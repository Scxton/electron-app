...
const paginatedResults = computed(() => {
    ...
    console.log('分页计算 - 开始索引:', startIndex);
    console.log('分页计算 - 结束索引:', endIndex);
    console.log('分页计算 - 当前页结果数量:', results.length);
    console.log('分页计算 - 页面大小设置:', pageSize.value);
    return results;
});
...
const handleDownload = async (item) => {
    console.log('开始下载成果:', item);
    try {
        // 默认用户ID为1
        const userId = 1;
        const achievementIds = [item.id];
        
        console.log('提交下载表单 - 成果ID:',typeof achievementIds);
        console.log('提交下载表单 - 用户ID:', typeof userId);
        ...
        if (fileNames && Array.isArray(fileNames) && fileNames.length > 0) {
            console.log('获取到的文件名列表:', fileNames);
...
const handleBatchDownload = async () => {
    ...
    console.log('开始批量下载 - 选中项目:', selectedItems.value);
    try {
        // 默认用户ID为1
        const userId = 1;
        const achievementIds = selectedItems.value;
        
        console.log('提交批量下载表单 - 成果ID列表:', achievementIds);
        console.log('提交批量下载表单 - 用户ID:', userId);
        ...
        if (fileNames && Array.isArray(fileNames) && fileNames.length > 0) {
            console.log('获取到的批量文件名列表:', fileNames);
        ...
        console.error('批量下载过程发生错误:', error);
        alert('批量下载失败，请稍后重试');
    }
};
...
const applyUserSettings = () => {
  ...
  console.log('已应用用户设置:', userSettings.value);
  console.log('当前页面大小:', pageSize.value);
  console.log('当前排序方式:', sortBy.value);
  console.log('当前优先类型:', priorityType.value);
};
...
const processSearchResults = (response) => {
    ...
    if (Array.isArray(response)){
        ...
        //调试信息
        console.log("allResults", searchResults.value);
        console.log('选中的成果类型:', selectedTypes.value);
        console.log('优先展示类型:', priorityType.value);
        console.log('过滤后的结果数量:', searchResults.value.length);
    }
};
...
const downloadAndSaveFile = async (fileNames) => {
    try {
        ...
        console.log('文件名列表类型:', typeof fileNames);
